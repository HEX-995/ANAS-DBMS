...
int lruPageFaults(int ref[], int n, int frames) {
    ...
    for (int i = 0; i < frames; i++) last[i] = -1;
...
int main() {
    ...
    int best = fifo;
    if (lru < best) best = lru;
    if (opt < best) best = opt;

    if (best == opt) printf("Most Efficient: Optimal\n");
    else if (best == lru) printf("Most Efficient: LRU\n");
    else printf("Most Efficient: FIFO\n");
